...
function fwds_scripts() {
    ...
    wp_register_script('jquerylib', 'http://code.jquery.com/jquery-1.11.1.min.js');
    // wp_enqueue_script('jquery');
    
    wp_register_script('slidesjs_core', plugins_url('js/jquery.scrollbox.js', __FILE__),array("jquerylib"));
...
function fwds_styles() {
     ...
     wp_register_style('slidesjs_example1', plugins_url('css/demo.css', __FILE__));
    wp_enqueue_style('slidesjs_example1');
}
...
function fwds_display_slider($attr, $content) {
    ...
    foreach ($gallery_images as $index => $gal_img) {
            if ($gal_img != "") {
            $html .= "<li><a href=''><img src='". $gallery_images[$index] ."'>";
    ...
    <button class="btn" id="demo5-forward"><img src="'.plugins_url( 'js-switch/images/arrow-left.png' ).'"></button>
    <button class="btn" id="demo5-backward"><img src="'.plugins_url( 'js-switch/images/arrow-right.png' ).'"></button>
  </div>';
...
function fwds_register_slider() {
    $labels = array(
        ...
        'singular_name' => _x( 'Js-Switch', 'slidesjs_slider' ),
        'add_new' => _x( 'Add Js-Switch', 'slidesjs_slider' ),
        'add_new_item' => _x('Add Js-Switch', 'slidesjs_slider' ),
        'edit_item' => _x( 'Edit Js-Switch', 'slidesjs_slider' ),
		'new_item' => _x( 'New Js-Switch', 'slidesjs_slider' )
    );
...
add_action('wp_enqueue_scripts', 'fwds_scripts1');

function fwds_scripts1() {
    global $post;

    wp_register_script('slidesjs_core1', plugins_url('js/hello.js', __FILE__),'','',true);
    wp_enqueue_script('slidesjs_core1');
